...
# Function to print in green (Linux)
print_green() {
    echo -e "\e[32m$1\e[0m"
}

# Function to append to HTML file (Linux)
append_to_html() {
    echo "$1" >> "$html_file"
}
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    powershell.exe -Command "& {
        ...
        $html_file = "${hostname}_${today_date}_${account_id}.html"
          ...
          # Start HTML file
        $html_content = @\"
<html><head><title>System Information</title><style>body { font-family: Arial, sans-serif; } h2 { color: green; } table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid black; padding: 8px; text-align: left; } th { background-color: #f2f2f2; }</